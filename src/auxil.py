...
NEAREST_FACT = .5    # portion of EK0 for 'nearest neighbour' coupling: between 0.177 and 1
...
def pinch(string, pre, post):
    '''selects the string between the first instance of substring (pre) and the last instance of substring (post)'''
...
def generateAdjDict(cells, spacing):
    ...
    cells satisfying 'adjacency' and appends these indices with the corresponding
    Ek values. First element of adjacency[i] is h_i '''
...
def generateHam(h,J, gamma=None):
    ...
    if type(gamma) in [int,float]:
        offdiag_flag=True
        gamma = [gamma]*N
        SX = [pauli(i+1,N,'x') for i in xrange(N)] # tril sparse matrix format
