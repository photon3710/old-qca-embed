#-------------------------------------------------------------------------------
# Name:         DenseInitialPlacement.py
# Purpose:      The input adjacency is a list of lists. The nth entry is a list of QCA cells, with cell numbers higher than n, that QCA cell n
#               is adjacent to. The main calling function, placeQubits(adjacency), returns a list whose nth value corresponds to the qubit that
#               QCA cell n is placed on. This version starts in the middle of the qubit array and favours density, with expansion direction being
#               randomly decided so that the qubits structure grows evenly from the middle outwards. This function is designed to handle situations
#               in which there are more adjacent QCA cells than number of corresponding available adjacent qubits.
#
# Author:       Michael Babcock
#
# Created:      11/07/2013
# Copyright:    (c) Michael Babcock 2013
#-------------------------------------------------------------------------------

#Takes ordered adjacency and order as inputs
#Calls Routing for every placement
...
def placeQubits(adjacency,order,numAdj,connect,tofile=True):
    rows=defineRows()
                    ...
                    if placed[currentCell] != 1:
                        ...
                        if len(placedAdjacent[currentCell]) == 1:
                            ...
                            while True:
                                ...
                                cost=RoutingFunctionv6.Routing([[qubits[placedAdjacent[currentCell][0]],qubits[placedAdjacent[currentCell][1]]]],couplers,routefile,tofile=tofile)
        ...
        while index != []:
            ...
            for i in range(len(index)):
                ...
                for j in range(len(newAdjacency[index[i]])):
                    if placed[newAdjacency[index[i]][j]]==0:
                        placed[newAdjacency[index[i]][j]]=1
                        #currentAC[index[i]]-=1
                        adjacentToRemove.append(index[i])
                        #currentAC[newAdjacency[index[i]][j]]-=1
                        adjacentToRemove.append(newAdjacency[index[i]][j])
                        toConnect[index[i]].pop(toConnect[index[i]].index(newAdjacency[index[i]][j]))
                        toConnect[newAdjacency[index[i]][j]].pop(toConnect[newAdjacency[index[i]][j]].index(index[i]))
                        #newOrder.append(newAdjacency[index[i]][j])
                        toDo.append(newAdjacency[index[i]][j])
                        toRemove1=[]
                        toRemove2=[]
                        for k in range(len(toConnect[newAdjacency[index[i]][j]])):
                            if placed[toConnect[newAdjacency[index[i]][j]][k]]==1:
                                #currentAC[newAdjacency[index[i]][j]]-=1
                                adjacentToRemove.append(newAdjacency[index[i]][j])
                                #currentAC[toConnect[newAdjacency[index[i]][j]][k]]-=1
                                adjacentToRemove.append(toConnect[newAdjacency[index[i]][j]][k])
                                toRemove1.append(newAdjacency[index[i]][j])
                                toRemove2.append(toConnect[newAdjacency[index[i]][j]][k])
                        for k in range(len(toRemove1)):
                            toConnect[toRemove1[k]].pop(toConnect[toRemove1[k]].index(toRemove2[k]))
                            toConnect[toRemove2[k]].pop(toConnect[toRemove2[k]].index(toRemove1[k]))
