...
def GtoCoef(G):
	...
	for node in nodes:
		h.append(0)
		if G.node[node].has_key('h'):
			h[-1]=G.node[node]['h']
		for key in G.edge[node]:
			J[nodes_key[node],nodes_key[key]]=G.edge[node][key]['J']
...
def routeWire(G,start,end,index,N=None):
	'''Adds nodes to graph G between labelled start and end nodes. If N is given,
	adds a wire of length N (N>=0), otherwise N is a random integer between 0 and
	WIRE_MAX. Returns next index'''
	...
	edges=[[start,index]]+[[index+i,index+i+1] for i in xrange(0,N-1)] + [[index+N-1,end]]
	G.add_edges_from(edges,J=-1)
...
def generateCircuit(max_count=MAX_COUNT, full_adj=True):
	...
	while go and index<MAX_COUNT:
		...
		if inputs and random()<.3:	# fill an input
			...
			for m in xrange(M):
				if inputs and random()<(len(inputs)/(len(inputs)+len(outputs))):	# route output to input
	...
	if not nx.is_connected(G):
		...
		Gs = sorted(nx.connected_component_subgraphs(G),key=lambda x: x.number_of_nodes())
		G=Gs[-1]
...
def generateCircuit2(full_adj=True):
	'''Generate a QCA circuit by randomly generating inverters and majority gates
	and then connecting outputs to inputs'''
